...
-- Description: Adiciona notificação WhatsApp para posts inelegíveis para boost em presets 017 e 018
-- Fluxo: filter_unsponsored → IF total >= 1 → IF total_inelegiveis >= 1 → WhatsApp avisar → Loop
-- Created: 2026-04-09
...
UPDATE presets SET
  nodes = '[
    ...
    {"id":"n06","type":"logic.if","label":"Tem posts elegíveis?","config":{"value":"1","operator":">=","variable":"total"},"position":{"x":230,"y":600}},
    {"id":"n07","type":"logic.stop","label":"Nenhum post novo","config":{},"position":{"x":450,"y":750}},
    {"id":"n14","type":"logic.if","label":"Tem posts inelegíveis?","config":{"value":"1","operator":">=","variable":"total_inelegiveis"},"position":{"x":80,"y":750}},
    {"id":"n15","type":"whatsapp.send_message","label":"Avisar posts inelegíveis","config":{"message":"⚠️ *Posts que não podem ser turbinados — {{cliente.nome}}*\n\n{{total_inelegiveis}} post(s) foram detectados como inelegíveis para boost via API.\n\nAcesse o Meta Business Suite e suba esses posts manualmente como anúncio."},"position":{"x":80,"y":900}},
    {"id":"n08","type":"logic.loop","label":"Loop","config":{"list":"{{posts}}"},"position":{"x":80,"y":1050}},
    {"id":"n09","type":"meta.create_ad","label":"Criar anúncio","config":{"creative_type":"instagram_post","source_instagram_media_id":"{{item.id}}","adset_id":"","campaign_id":"","name":"Post Instagram {{item.id}}"},"position":{"x":80,"y":1200}},
    {"id":"n10","type":"logic.if","label":"Anúncio criado?","config":{"operator":"not_empty","variable":"ad_id"},"position":{"x":80,"y":1350}},
    {"id":"n11","type":"whatsapp.send_message","label":"Notificar erro","config":{"message":"❌ *Falha ao criar anúncio {{cliente.nome}}*\n\n🔗 Post: {{post_permalink}}\n📝 Legenda: {{post_caption}}\n📸 Tipo: {{post_media_type}}\n\n⚠️ Motivo: {{motivo}}\n🛠️ Erro técnico: {{erro_meta}}\n\n▶️ Acesse o post para subir manualmente."},"position":{"x":300,"y":1500}},
    {"id":"n13","type":"whatsapp.send_message","label":"Notificar sucesso","config":{"message":"✅ *Anúncio criado — {{cliente.nome}}*\n\n🔗 {{post_permalink}}\n📝 {{post_caption}}"},"position":{"x":-150,"y":1500}},
    {"id":"n12","type":"whatsapp.send_message","label":"Loop concluído","config":{"message":"✅ *Loop de anúncios — {{cliente.nome}}*\n\n📊 Posts processados: {{loop_total}}"},"position":{"x":580,"y":1050}}
  ]'::jsonb,
  edges = '[
    ...
    {"id":"e06","source":"n06","target":"n14","sourceHandle":"yes","targetHandle":"default"},
    {"id":"e07","source":"n06","target":"n07","sourceHandle":"no","targetHandle":"default"},
    {"id":"e14a","source":"n14","target":"n15","sourceHandle":"yes","targetHandle":"default"},
    {"id":"e14b","source":"n14","target":"n08","sourceHandle":"no","targetHandle":"default"},
    {"id":"e15a","source":"n15","target":"n08","sourceHandle":"default","targetHandle":"default"},
...
UPDATE presets SET
  nodes = '[
    ...
    {"id":"s06","type":"logic.if","label":"Tem posts elegíveis?","config":{"value":"1","operator":">=","variable":"total"},"position":{"x":300,"y":600}},
    {"id":"s07","type":"logic.stop","label":"Nenhum post novo","config":{},"position":{"x":500,"y":750}},
    {"id":"s17","type":"logic.if","label":"Tem posts inelegíveis?","config":{"value":"1","operator":">=","variable":"total_inelegiveis"},"position":{"x":100,"y":750}},
    {"id":"s18","type":"whatsapp.send_message","label":"Avisar posts inelegíveis","config":{"message":"⚠️ *Posts que não podem ser turbinados — {{cliente.nome}}*\n\n{{total_inelegiveis}} post(s) foram detectados como inelegíveis para boost via API.\n\nAcesse o Meta Business Suite e suba esses posts manualmente como anúncio."},"position":{"x":100,"y":900}},
    {"id":"s08","type":"logic.loop","label":"Loop","config":{"list":"{{posts}}"},"position":{"x":100,"y":1050}},
    {"id":"s09","type":"logic.switch","label":"Verificar palavra-chave","config":{"variable":"{{item.caption}}","cases":[{"id":"case_a","label":"Caso 1","value":"palavra1"},{"id":"case_b","label":"Caso 2","value":"palavra2"}]},"position":{"x":100,"y":1200}},
    {"id":"s10","type":"meta.create_ad","label":"Criar anúncio Caso 1","config":{"creative_type":"instagram_post","source_instagram_media_id":"{{item.id}}","adset_id":"","campaign_id":"","name":"Post {{item.id}} - Caso 1"},"position":{"x":-200,"y":1360}},
    {"id":"s11","type":"meta.create_ad","label":"Criar anúncio Caso 2","config":{"creative_type":"instagram_post","source_instagram_media_id":"{{item.id}}","adset_id":"","campaign_id":"","name":"Post {{item.id}} - Caso 2"},"position":{"x":100,"y":1360}},
    {"id":"s12","type":"meta.create_ad","label":"Criar anúncio Padrão","config":{"creative_type":"instagram_post","source_instagram_media_id":"{{item.id}}","adset_id":"","campaign_id":"","name":"Post {{item.id}} - Padrão"},"position":{"x":400,"y":1360}},
    {"id":"s13","type":"logic.if","label":"Anúncio criado?","config":{"operator":"not_empty","variable":"ad_id"},"position":{"x":100,"y":1520}},
    {"id":"s14","type":"whatsapp.send_message","label":"Notificar erro","config":{"message":"❌ *Falha ao criar anúncio {{cliente.nome}}*\n\n🔗 Post: {{post_permalink}}\n📝 Legenda: {{post_caption}}\n📸 Tipo: {{post_media_type}}\n\n⚠️ Motivo: {{motivo}}\n🛠️ Erro técnico: {{erro_meta}}\n\n▶️ Acesse o post para subir manualmente."},"position":{"x":350,"y":1680}},
    {"id":"s16","type":"whatsapp.send_message","label":"Notificar sucesso","config":{"message":"✅ *Anúncio criado — {{cliente.nome}}*\n\n🔗 {{post_permalink}}\n📝 {{post_caption}}"},"position":{"x":-150,"y":1680}},
    {"id":"s15","type":"whatsapp.send_message","label":"Loop concluído","config":{"message":"✅ *Loop de anúncios — {{cliente.nome}}*\n\n📊 Posts processados: {{loop_total}}"},"position":{"x":650,"y":1050}}
  ]'::jsonb,
  edges = '[
    ...
    {"id":"se06","source":"s06","target":"s17","sourceHandle":"yes","targetHandle":"default"},
    {"id":"se07","source":"s06","target":"s07","sourceHandle":"no","targetHandle":"default"},
    {"id":"se17a","source":"s17","target":"s18","sourceHandle":"yes","targetHandle":"default"},
    {"id":"se17b","source":"s17","target":"s08","sourceHandle":"no","targetHandle":"default"},
    {"id":"se18a","source":"s18","target":"s08","sourceHandle":"default","targetHandle":"default"},
